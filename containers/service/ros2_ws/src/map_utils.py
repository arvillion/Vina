...
def convert_text2cordinate(gpt_endpoint, query_text, system_prompt, map_image, origin, resolution, window_size, free_points):
    try:
        result_from_gpt = requests.post(gpt_endpoint, json={"instruction": system_prompt, "prompt": query_text} )
...
def convert_image2pose(vpr_endpoint, use_depth = False):
    ...
    result = requests.post(vpr_endpoint,files=files)
    result = result.json()
